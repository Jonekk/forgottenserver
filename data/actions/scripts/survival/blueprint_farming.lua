FARMING_BLUEPRINT_READY_ID = 26410
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if weightChange > 0 and player:getFreeCapacity() < weightChange then
        player:sendTextMessage(MESSAGE_STATUS_SMALL, ("Not enough capacity to work on that (%d required for next step)"):format(weightChange))
        return true
    end

    if player:useStamina(workOnUse) then
        player:addSkillTries(SKILL_FARMING, workOnUse)
        if workAfterUse == workRequired then
            item:transform(FARMING_BLUEPRINT_READY_ID)
            ...
            player:sendTextMessage(MESSAGE_STATUS_SMALL, ("Work done: %d/%d"):format(workAfterUse, workRequired))
